...
class SearchHandler:
    ...
    def __init__(
        ...
        :param hdhive_client: 保留参数，兼容旧调用；当前 HDHive 使用 Web/Cookie 客户端
    ...
    def get_enabled_sources(self) -> List[str]:
        ...
        if self._hdhive_enabled:
            # Web/Cookie 链路：优先使用 Cookie；也支持用户名密码登录后查询。
    ...
    def search_resources(
        ...
        搜索优先级: 默认 HDHive > PanSou，支持通过配置自定义排序
    ...
    def _search_hdhive(
        ...
        """
        使用 HDHive 搜索资源
        当前统一使用 Web/Cookie 链路：TMDB 跳转页 -> 详情页 RSC groupData["115"]。
        ...
        # 统一改用已跑通的 Web 链路：TMDB 跳转页 -> 详情页 RSC groupData["115"]。
    ...
    def _search_hdhive_api(self, mediainfo: MediaInfo, hdhive_media_type: str) -> List[Dict]:
        """
        使用 Web Cookie 模式查询 HDHive 资源。
        新链路不再依赖 OpenAPI：/tmdb/{type}/{tmdb_id} -> /movie|tv/{page_id} -> RSC groupData["115"]。
        """
        from ..clients import HDHiveOpenAPIError, HDHiveWebClient

        if not (self._hdhive_cookie or (self._hdhive_username and self._hdhive_password)):
            logger.warning("HDHive Web 模式需要配置 Cookie，或配置用户名和密码用于登录")
            return []

        hdhive_client = HDHiveWebClient(
            username=self._hdhive_username,
            password=self._hdhive_password,
            cookie=self._hdhive_cookie,
            proxy=settings.PROXY,
    ...
    def unlock_hdhive_resource(self, slug: str, unlock_points: int) -> Optional[str]:
        ...
        if not self._hdhive_client or not getattr(self._hdhive_client, "is_ready", False):
            if self._hdhive_cookie or (self._hdhive_username and self._hdhive_password):
                self._hdhive_client = HDHiveWebClient(
                    username=self._hdhive_username,
                    password=self._hdhive_password,
                    cookie=self._hdhive_cookie,
                    proxy=settings.PROXY,
                ...
                logger.warning("HDHive Web 模式需要配置 Cookie，或配置用户名和密码用于登录后解锁")
                return None
