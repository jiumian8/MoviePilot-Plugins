...
from .clients import PanSouClient, P115ClientManager, HDHiveWebClient
...
class P115panjiumian(_PluginBase):
    ...
    # 搜索源优先级（按列表顺序），为空时默认 HDHive > PanSou
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._hdhive_account_username = config.get("hdhive_account_username", "")
            self._hdhive_account_points = str(config.get("hdhive_account_points", "") or "")
    ...
    def _init_clients(self):
        ...
        # HDHive 使用 Web/Cookie 模式，不再需要 OpenAPI/OAuth 配置。
        self._hdhive_client = None
        if self._hdhive_enabled:
            if self._hdhive_cookie or (self._hdhive_username and self._hdhive_password):
                self._refresh_hdhive_account_info(proxy)
                logger.info("HDHive 配置已加载（Web/Cookie 模式）")
            else:
                logger.warning("HDHive 已启用但未配置 Cookie 或用户名密码，将无法使用 HDHive 查询功能")

        if self._cookies:
            self._p115_manager = P115ClientManager(cookies=self._cookies)

    def _refresh_hdhive_account_info(self, proxy=None):
        """刷新 HDHive 用户名和剩余积分，用于配置界面只读展示。"""
        try:
            client = HDHiveWebClient(
            ...
            username = info.get("display_name") or info.get("username") or ""
            points = info.get("points")
            self._hdhive_account_username = str(username or "")
            self._hdhive_account_points = str(points if points is not None else "")
            if self._hdhive_account_username or self._hdhive_account_points:
                logger.info(f"HDHive 账号信息：{self._hdhive_account_username or '-'}，剩余积分 {self._hdhive_account_points or '-'}")
                self.__update_config()
        except Exception as e:
            logger.warning(f"HDHive 账号信息刷新失败：{e}")
    ...
    def __update_config(self):
        self.update_config({
            ...
            "hdhive_max_unlock_points": self._hdhive_max_unlock_points,
            "hdhive_max_points_per_sub": self._hdhive_max_points_per_sub,
            "hdhive_account_username": self._hdhive_account_username,
            "hdhive_account_points": self._hdhive_account_points,
            "hdhive_username": self._hdhive_username,
            "hdhive_password": self._hdhive_password,
            "hdhive_cookie": self._hdhive_cookie,
